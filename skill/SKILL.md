...
- [x] `bun add -D @typescript/native-preview oxlint oxfmt`
- [x] `package.json` punya script `check:fast`, `check`, `fix`
- [x] VS Code setting `"typescript.experimental.useTsgo": true`
- [x] File `.oxfmtrc.jsonc` ada dan `printWidth` diset (opsional)
- [x] Oxlint: `bunx oxlint --init` atau `bunx @oxlint/migrate ...`
- [x] (opsional) pre-commit / CI menjalankan `bun run check:fast` / `bun run check`
